...
export const mostrarMensaje = (mensaje, tipo = 'info') => {
  ...
  setTimeout(() => { div.style.transform='translateY(80px)'; div.style.opacity='0'; }, 3000);
};

export const actualizarBadgeCarrito = () => {
  const datos = localStorage.getItem('carritoTechStore');
...
export const inyectarBadge = () => {
  document.querySelectorAll('nav a[href="carrito.html"]').forEach(a => {
    if (!a.querySelector('.carrito-badge')) {
      ...
      span.style.cssText = 'display:none;background:#F59E0B;color:#fff;font-size:0.65rem;font-weight:700;border-radius:50%;width:18px;height:18px;align-items:center;justify-content:center;margin-left:4px;vertical-align:middle;';
      a.appendChild(span);
    }
  });
